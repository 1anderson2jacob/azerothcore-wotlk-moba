...
1. In-game, walk the intended route and `.gps` at points along it. Note
   consecutive points need to stay **well under 30 yards apart** (ideally
   ~5 yards) — see the leash-radius gotcha below for why. Doing this by
   hand for many points is exactly what the (planned) waypoint-generator
   script is for; until it exists, interpolate manually.
2. `UPDATE waypoint_data SET position_x = .., position_y = .., position_z = .. WHERE id = <path id> AND point = <point number>;`
   (or `DELETE`+`INSERT` the whole path if the point count is changing)
3. Remember: melee-left/melee-right/siege each have their **own** path ID
   (not shared with the base lane path caster uses) — see "How the creep
   wave system works" above for why. Moving the lane means updating all
   the per-slot paths, not just one.
4. Apply the SQL, restart worldserver, `.debug bg`, queue, confirm.
