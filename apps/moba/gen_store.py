...
def emit_catalog(menu_rows, grant_rows, tabs_meta):
    """Generated Lua catalog for the shop addon, plus a per-map version hash.
    ...
    wrong grant -- but it CAN display a wrong price. That is what the hash is for:
    the same value lands in mod_moba_store_version, the server sends it at the
    handshake, and the panel refuses to draw a catalog the server does not know.

    The hash covers the emitted TEXT of a map's block, so anything a player could
    see changes it. A pure formatting change in this generator bumps it too, which
    ...
    hashes = {map_id: hashlib.sha1("\n".join(b).encode("utf-8")).hexdigest()[:12]
              for map_id, b in blocks.items()}
    ...
    for map_id in sorted(hashes):
        lines.append(f'  [{map_id}] = "{hashes[map_id]}",')
    lines.append("}")
...
def main():
    ...
    # Catalog first: the SQL carries its hashes, so the Lua has to be built before it.
    catalog_text, catalog_hashes = emit_catalog(menu_rows, grant_rows, tabs_meta)
    ...
    for map_id in sorted(catalog_hashes):
        print(f"  catalog version map {map_id}: {catalog_hashes[map_id]}")
    if item_copies:
    ...
    print("Restart worldserver -- mod_moba_store.sql auto-applies on boot, and the "
          ...
          "Catalog.lua ships with it and is never sent over the wire. Skipping the "
          "recopy now shows an out-of-date notice instead of wrong prices.")
