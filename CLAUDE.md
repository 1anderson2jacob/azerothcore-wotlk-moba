...
- **Client-patch bundle** — the DBC/MPQ pipeline is live now (`apps/moba/wmo/dbc_tool.py` plus step 7 of `apps/moba/wmo/README.md`), so these are unbuilt rather than blocked: the recall tooltip still reads "Returns you to \<bind\>"; recall and fountain have no custom spell visuals; custom battle sounds (a doors-open cue and a first-wave-only cue — two `PlaySoundToAll` calls, ~10 min once `SoundEntries.dbc` rows exist); Twisted Treeline music; and **drop-item copies, which retire the match-granted ledger**. The shop's `custom_items` copies ship — server rows, manifest and `Item.dbc` rows — so shop tooltips carry a correct per-entry `SellPrice`; drops still hand out stock entries.
