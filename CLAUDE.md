...
- **Resurrection** — LoL-style: per-player death timer that starts on Release Spirit (not on death), fixed respawn at the team's start position, no shared-pulse graveyard res. `src/server/scripts/Custom/moba_resurrection.cpp` (`OnPlayerReleasedGhost` hook) → `BattlegroundMOBA::StartRespawnTimer`; countdown + revive in `PostUpdateImpl`. Timing is **map-keyed** config: `data/sql/custom/mod_moba_resurrection.sql` → `MobaResurrectionData.{h,cpp}` (`BaseMs + PerMinMs × match-minutes`, capped; match-time measured from doors-open). Spirit-guide NPCs removed so the base revive queue never populates; graveyard rows 1103/1104 kept — they're the BG start-loc / spawn bubble that respawn reuses.
...
Next (in order):
1. **Map-key the tower + creep tables** (and their generators) — retrofit `mod_moba_tower_data` / `mod_moba_creep_data` + `MobaTowerData`/`MobaCreepData` to key by map id, matching `mod_moba_resurrection`, so all MOBA content is partitioned per map (toward multiple concurrent maps/modes).
